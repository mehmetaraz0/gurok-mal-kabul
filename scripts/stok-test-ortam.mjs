...
export const TEMEL_SEMA = `
  ...
  grant insert, update on public.sayim_oturumlari, public.sayim_detaylari,
    public.stok, public.stok_hareketleri to authenticated;
`;

// Ekranin yazma yollari RPC kullanir; uretimdeki imzalarin yerel karsiligi.
export const RPC_SEMA = `
  create or replace function public.stok_ekle(p_urun_kodu text, p_depo_kodu text,
    p_otel_id text, p_delta numeric)
  returns numeric language plpgsql security invoker set search_path = public as $$
  declare v_yeni numeric;
  begin
    insert into public.stok (urun_kodu, depo_kodu, otel_id, miktar)
      values (p_urun_kodu, p_depo_kodu, p_otel_id, greatest(p_delta, 0))
      on conflict do nothing;
    update public.stok set miktar = greatest(miktar + p_delta, 0),
           guncelleme_tarihi = now()
     where urun_kodu = p_urun_kodu and depo_kodu = p_depo_kodu
     returning miktar into v_yeni;
    return coalesce(v_yeni, 0);
  end $$;
  ...
  create or replace function public.stok_transfer(p_urun_kodu text, p_kaynak_depo text,
    p_hedef_depo text, p_hedef_otel text, p_miktar numeric)
  returns void language plpgsql security invoker set search_path = public as $$
  begin
    update public.stok set miktar = greatest(miktar - p_miktar, 0)
     where urun_kodu = p_urun_kodu and depo_kodu = p_kaynak_depo;
    insert into public.stok (urun_kodu, depo_kodu, otel_id, miktar)
      select p_urun_kodu, p_hedef_depo, p_hedef_otel, 0
       where not exists (select 1 from public.stok
                          where urun_kodu = p_urun_kodu and depo_kodu = p_hedef_depo);
    update public.stok set miktar = miktar + p_miktar
     where urun_kodu = p_urun_kodu and depo_kodu = p_hedef_depo;
  end $$;
