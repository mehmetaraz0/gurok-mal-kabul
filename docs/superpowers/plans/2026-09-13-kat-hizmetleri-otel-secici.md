...
-- Rol b0…a7, bu dosyada pms_housekeeping için `tam` yetkisi verilen QA rolüdür.
-- ---------------------------------------------------------------------------
...
`kullanicilar.id` ile `auth_user_id` bu dosyada **ayrı** uuid uzaylarıdır (`c0…` ve `a0…`); PIN haritası `auth_user_id` değerini kullanır.
