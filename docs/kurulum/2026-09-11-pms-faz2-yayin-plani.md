...
> **DÜZELTME (2026-09-13):** yukarıdaki "kimse giriş yapamaz" tespiti, auth yedeği eklenmeden önceki durumu anlatır ve tarihsel kayıt olarak duruyor. Aynı gün yedeğin kapsamına `auth` şeması eklendi ve provaya yedinci aşama kondu: üretim yedeğiyle **13/13 kimlik yedekten geldi, yükleme hatası 0**. Artık proje tümden kaybında iş verisi **ve kimlikler** geri gelir; kalan sınır, hedef projenin `auth` şemasının platform tarafından sağlanması ve GoTrue sürüm uyumudur. Tasarım: `docs/superpowers/specs/2026-09-13-auth-yedegi-design.md`.

Sayaç dosyası bu boşluğu sayıya çevirir (`kurtarma_kapsami`): kaç ayrı kimlik gerekiyor, kaçı aktif kullanıcıya ait, `auth` şemasında bugün kaç tablo var. Prova aynı ölçümü geri yüklenen kopyada tekrarlar ve **veri yedeğinden kaç kimlik geldiğini** yazar — veri yedeği tasarımı gereği kimlik taşımaz, o yüzden bu sayı 0'dır ve prova eksikleri yalnız kendi yürüyebilmesi için üretip raporlar. **2026-09-13'ten beri** asıl kanıt yedinci aşamadadır: ayrı auth yedeği yüklenir ve kimliklerin **yedekten** geldiği ölçülür (ölçülen: 13/13).
